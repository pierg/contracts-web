...
export default class ComponentsView extends React.Component {
  ...
  editComponent = (i) => {
    ...
    for (let i = 0; i < component.assumptions.LTL.length; i++) {
      tmpComponent.assumptions.push({
        LTL: [component.assumptions.LTL[i]],
        PL: [""],
        type: "LTL",
      });
    }
    for (let i = 0; i < component.assumptions.PL.length; i++) {
      tmpComponent.assumptions.push({
        PL: [component.assumptions.PL[i]],
        LTL: [""],
        type: "PL",
      });
    }
    for (let i = 0; i < component.guarantees.LTL.length; i++) {
      tmpComponent.guarantees.push({
        LTL: [component.guarantees.LTL[i]],
        PL: [""],
        type: "LTL",
      });
    }
  ...
  uploadComponent = (component) => {
    if (component) {
      if (
        component.hasOwnProperty("guarantees") &&
        component.hasOwnProperty("name") &&
        component.hasOwnProperty("description")
      ) {
        setTimeout(() => {
  ...
  selectAllComponents = (e) => {
    if (e.target.checked) this.props.setSelectedComponents(this.props.components);
    else this.props.setSelectedComponents([]);
  };
  ...
  render() {
    ...
    for (let i = 0; i < this.props.components.length; i++) {
      ...
      components.push(
        <tr key={i} onClick={() => this.lineClicked(i)} className={lineClass}>
          <td className="pr-5">{this.props.components[i].name}</td>
          <td>{this.reduceDescription(this.props.components[i].description)}</td>
          <td className="float-right border-0">
            <Tooltip html="info" position="top" arrow="true">
              <Button
                size="sm"
                color="gray"
                onClick={(e) => {
    ...
    return (
      <>
        <div className="px-3 pb-3 w-7/12 relative flex flex-col flex-initial min-w-component break-words bg-white rounded shadow-md m-auto">
          ...
          {this.props.selectedLibrary === null ? (
            <div className="flex justify-center my-20 py-3 text-blueGray-500">
              Select a library
            </div>
          ) : (
          ...
          {this.props.selectedLibrary !== null && (
            <div className="text-right">
              Selected components : {this.props.selectedComponents.length}
            </div>
          )}
        </div>
