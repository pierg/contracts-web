...
function ContractsConnectInstances(props) {
    ...
    let instances = [];
    let lineClass =
        "flex flex-row relative text-lg p-3 rounded hover:bg-blueGray-200 text-blueGray-700 hover:text-blueGray-900 cursor-pointer";
    let subtree;
    let node;
        ...
        instances.push(
            <li key={i} className="border-b-1">
                <div className="flex flex-col justify-between">
                    <div className={lineClass} onClick={() => lineClicked(i)}>
                        <div className="mr-2">{props.instances[i].name}</div>
                        {!props.selectedLibrary.default && (
                            <div className="absolute right-0 mr-2">
                                <Tooltip title="Delete instance" position="right" arrow="true">
                                    <Button
        ...
        <span className={contractsconnect.instances.instanceStyle}>
          {contractsconnect.instances.instancesTitle}
            ...
            {props.selectedLibrary.default &&
                <Tooltip
                    html={
                        <div className="flex flex-col text-center">
                            {contractsconnect.instances.demoMessage.map((prop, key) => (
                                <div className="mb-2" key={key}>
                                    <div>{prop.content}</div>
                                </div>
                            ))}
                ...
                {!props.selectedLibrary.default && (
                    <Tooltip title="Clear all instances" position="right" arrow="true">
                        <Button
