...
function ComponentsDiagram(props) {
  const CustomInstance = (instance) => {
    ...
    for (let i = 0; i < instance.inputs.length; i++) {
      inputs.push(
        <div key={"i-" + i} className="flex flex-row">
          <div>{instance.inputs[i].props.id.split("__")[1]}</div>
          <div className="pl-1">
    ...
    for (let i = 0; i < instance.outputs.length; i++) {
      outputs.push(
        <div key={"o-" + i} className="flex flex-row">
          <div>{instance.outputs[i].props.id.split("__")[1]}</div>
          <div className="pl-1">
    ...
    return (
      <div
        style={{ background: "#DAE1E7" }}
        className="border rounded-xl border-blueGray-400"
      >
        <div
        ...
        <div className="flex flex-row justify-evenly p-2">
          <div className="flex-col pr-2">
            <div className="fs-6 pb-1">INPUTS</div>
            {inputs}
          </div>
          ...
          <div className="flex-col pl-2">
            <div className="fs-6 pb-1">OUTPUTS</div>
            {outputs}
          </div>
        </div>
      </div>
    ...
    return (
      <div className="border rounded-xl border-blueGray-400 bg-lightBlue-200">
        ...
        <div className="flex flex-row my-2">
          <div className="flex-col">{colPortInput}</div>
          <div className="flex-col w-50">{colNameInput}</div>
          <div className="flex-col w-50 text-right">{colNameOutput}</div>
          <div className="flex-col">{colPortOutput}</div>
        </div>
      </div>
    );
  };
  ...
  for (let i = 0; i < props.instances.length; i++) {
    ...
    for (let j = 0; j < props.instances[i].inputs.length; j++) {
      ...
      put.id =
        j +
        "__" +
        props.instances[i].inputs[j].name +
        "__" +
        props.instances[i].inputs[j].type;
    ...
    for (let j = 0; j < props.instances[i].outputs.length; j++) {
      ...
      put.id =
        j +
        "__" +
        props.instances[i].outputs[j].name +
        "__" +
        props.instances[i].outputs[j].type;
  ...
  for (let i = 0; i < props.connections.length; i++) {
    ...
    if (props.connections[i].boolUniquePort) {
      ...
      node.coordinates = [
        200 + (cptDoubleLink % 4) * distancePos,
        300 + Math.floor(cptDoubleLink / 4) * 300,
      ];
      node.inputs = [];
      node.outputs = [];
      portIn = props.connections[i].connectors[0].split("-")[0];
        ...
        <div className="px-3 pb-3  flex flex-col min-w-0 break-words bg-white rounded shadow-md m-auto">
          <div className=" flex justify-center text-2xl m-3 ">
            ...
            <div className="flex justify-center pr-10">
              <Tooltip title="Upload a design" position="right" arrow="true">
                ...
                >
                  <i
                    className={
                      componentInfo.info.texts.component.header.uploadButton.icon
                    }
                  ></i>
                </Button>
              </Tooltip>
