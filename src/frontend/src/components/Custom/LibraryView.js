...
function LibraryView(props) {
  ...
  for (let i = 0; i < props.libraries.length; i++) {
    libraryClass =
      "border-b-1 align-middle flex justify-between text-blueGray-700 text-base p-2 sm:p-3 cursor-pointer";
              ...
              <Tooltip
                html={
                  <div className="flex flex-col text-center ">
                    {librariesviewsinfo.demoLibrary.message.map((prop, key) => (
                ...
                arrow="true"
              >
                <i className={librariesviewsinfo.demoLibrary.icon} />
              </Tooltip>
            </div>
          </>
        )}
  ...
  return (
    <div className="px-3 pb-3 w-3/12 relative flex flex-col flex-initial min-w-library break-words bg-white rounded shadow-md h-fit mb-5 mx-5">
      ...
      <div className="max-h-400-px mt-3 mx-4 mb-4">
        <ul>{tmp_libraries}</ul>
      </div>
    </div>
  );
