...
function ComponentInfoOffCanvas(props) {
    ...
    if (props.component) {
        for (let i=0; i<props.component.inputs.length; i++) {
            inputs.push(<div className="pb-2" key={"inputs"+i}>{props.component.inputs[i].name + " (" + props.component.inputs[i].type+")"}</div>)
        }
        ...
        <Offcanvas.Body>
            {props.component && (
                <div className="m-auto">
                    <div className="py-3 text-md text-justify">{props.component.description}</div>
