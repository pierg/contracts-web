...
function ComponentsToInstances(props) {
  ...
  const addComponentsToInstances = (component) => {
    ...
    if (componentsToInstancesTmp.includes(component)) {
      componentsToInstancesTmp = componentsToInstancesTmp.filter(
        (e) => e !== component
      );
    } else {
  ...
  return (
    <>
      ...
      <ModalBody>
        <div className="fs-6 mb-2">Select one or more components</div>
            ...
            {componentsClass.map((prop, key) => {
              return (
                <li
                  ...
                  className={prop}
                  onClick={() =>
                    addComponentsToInstances(props.selectedComponents[key])
                  }
                >
                  <div className="flex justify-between">
                    <div>{props.selectedComponents[key].name}</div>
                  </div>
                </li>
              );
            })}
      ...
      <ModalFooter>
        <div className="flex flex-col w-full">
          <div className="flex w-full justify-between">
            ...
            <Button color="info" onClick={addInstances}>
              Add
            </Button>
          </div>
        </div>
